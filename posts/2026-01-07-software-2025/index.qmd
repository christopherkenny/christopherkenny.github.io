...
As such, every package received at least one update to reflect this change.
In total, my existing packages receieved *35* CRAN submissions in 2025.
...
As with many R bad experiences, Brian Ripley caused this one.
`geomander` was due for an update to address a CRAN note and, *prior to the deadline*, he yanked the package from CRAN and archived its dependents.
This led to a flurry of submissions to get everything back on CRAN before the holiday break.
...
Finally, there's always some work going on that hasn't made it to GitHub yet.
A few notable projects in the works include:

### `redistio`

::: {.column-margin}
![](../../hexes/hex_redistio.png)
:::
...
[`ha`](https://christophertkenny.com/ha/) provides access to the HomeAssistant API so that you can change the colors of the lights or play music or anything else without ever leaving RStudio or Positron.
